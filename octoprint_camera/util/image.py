...
def imdecode(stream, flags=cv2.IMREAD_COLOR):
    ...
    if not streamval:
        return
    return cv2.imdecode(np.fromstring(streamval, np.int8), flags)
...
def corner_settings_valid(settings):
    ...
    from octoprint_mrbeam.camera.config import (
        ...
        get_corner_calibration,
    )

    return isinstance(settings, Mapping) and is_corner_calibration(
        get_corner_calibration(settings)
    )
