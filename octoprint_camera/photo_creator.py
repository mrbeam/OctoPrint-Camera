...
from octoprint_camera.camera import gaussBlurDiff, save_debug_img

# from octoprint_camera.camera
...
class PhotoCreator(object):
    def __init__(self, _plugin, _plugin_manager, path, debug=False):
        ...
        self._analytics_handler = None  # _plugin.analytics_handler#todo jsoef
        ...
        self._logger = mrb_logger(
            "octoprint.plugins.camera.photo_creator.PhotoCreator", lvl=_level
        )
        ...
        self._logger.info("path for images %s", self.final_image_path)
    ...
    def serve_pictures(
        ...
        # 500 x 390, 75% JPEG 65quality ~ 60 kB
        ...
        cam.wait()
        while self._plugin.cameraHandler._lid_closed:
            # Wait for the lid to be completely open
            if self._plugin.cameraHandler._interlock_closed or self.stopping:
                return
            time.sleep(0.2)
        ...
        while not self.stopping:
            ...
            if self.refresh_pic_settings.isSet():
                ...
                path_to_lens_calib = self._plugin.cameraHandler.get_calibration_file()
            ...
            if self._plugin.cameraHandler.lensCalibrationStarted:
                ...
                while self._plugin.cameraHandler.lensCalibrationStarted:
            ...
            if latest is None:
                ...
                self._plugin._logger.debug("The last picture is empty")
                continue
            if self.stopping:
            ...
            if self._plugin.cameraHandler.need_corner_calibration():
        ...
        self._plugin._logger.debug("PhotoCreator_stopping")
    ...
    def send_last_img_to_analytics(
        ...
        dist = ""
        path_to_cam_params = self._plugin.cameraHandler.get_calibration_file()
        try:
