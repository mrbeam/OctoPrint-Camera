    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            self.def_pic = settings().get(
                ["camera", "mock", "img_static"], defaults=settings().get(["webcam"])
            )
            self.def_folder = settings().get(["camera", "mock", "img_folder"])
        except ValueError:
            sett = settings(init=True)
            self.def_pic = sett.get(
                ["camera", "mock", "img_static"], defaults=sett.get(["webcam"])
            )
            self.def_folder = sett.get(["camera", "mock", "img_folder"])
